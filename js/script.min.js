! function(e) {
    ...
    function d() {
        b(), "gdpr" === n._bannerConfig.settings.applicableLaw && !0 === n._bannerConfig.config.revisitConsent.status && j()
    }
    ...
    function y() {
        ...
        function() {
            ...
            T(t, "close-button"), T(t, "detail-close"), T(t, "detail-powered-by"), T(t, "revisit-consent"), T(t, "optout-powered-by"),
                function(e) {
                ...
                function() {
                    ...
                    a("=settings-button", () => E("settings-button")), a("=detail-close", () => v()), a("=optout-cancel-button", () => v()), a("=close-button", () => (c._ckySetInStore("action", "yes"), void d())), a("=donotsell-button", () => E("donotsell-button")), a("=reject-button", x("reject")), a("=accept-button", x("all")), a("=detail-accept-button", x("all")), a("=detail-save-button", x()), a("=detail-category-preview-save-button", x()), a("=optout-confirm-button", x()), a("=detail-reject-button", x("reject")), a("=revisit-consent", () => _revisitCkyConsent()),
                        function() {
                            ...
                            e.map(t => {
                                ...
                                a(n, ({
                                    ...
                                    o !== "ckySwitch" + t && function() {
                                        return u("toggle", ...arguments)
                                    }(n, "cky-accordion-active", !1) && e.filter(e => e !== t).map(e => function() {
                                        return u("remove", ...arguments)
                                    }("#ckyDetailCategory" + e, "cky-accordion-active", !1))
                                })
                            })
                        }()
                }(),
                ...
                function() {
                    let e = "flex-end";
                    ["detail-powered-by", "optout-powered-by"].map(t => {
                        const n = document.querySelector(`[data-cky-tag="${t}"]`);
                        n && ("optout-powered-by" === t && (e = "center"), n.style.display = "flex", n.style.justifyContent = e, n.style.alignItems = "center")
                    })
                }()
        }(),
        function() {
            if ("gdpr" !== p()) return;
            const e = n._shortCodes.find(e => "cky_show_desc" === e.key),
                t = n._shortCodes.find(e => "cky_hide_desc" === e.key);
            if (!e || !t) return;
            const o = t.content,
                c = e.content,
                r = window.innerWidth < 376 ? 150 : 300,
                s = document.querySelector('[data-cky-tag="detail-description"]');
            if (s.textContent.length < r) return;
            const i = s.innerHTML,
                u = (new DOMParser).parseFromString(i, "text/html").querySelectorAll("body > p");
            if (u.length <= 1) return;
            let l = "";
            for (let e = 0; e < u.length; e++) {
                if (e === u.length - 1) return;
                const t = u[e];
                if (`${l}${t.outerHTML}`.length > r && t.insertAdjacentHTML("beforeend", "...&nbsp;" + c), l = `${l}${t.outerHTML}`, l.length > r) break
            }

            function d() {
                s.innerHTML = `${i}${o}`, a("=hide-desc-button", y)
            }

            function y() {
                s.innerHTML = l, a("=show-desc-button", d)
            }
            y()
            ...
            let o = "yes";
            for (const r of n._categories) "gdpr" !== e || r.isNecessary || r.defaultConsent[e] || (o = "no"), "no" === o ? t.rejected.push(r.slug) : t.accepted.push(r.slug), c._ckySetInStore("" + r.slug, o);
            O()
        }(), function() {
    ...
    async function f() {
        try {
            y(), document.querySelector("body").addEventListener("click", e => {
    ...
    function h() {
        if ("classic" === k()) return m();
        let e = "ccpa" === p() ? i("optout-popup") : i("detail");
        return e && e.closest(".cky-modal") || !1
    }

    function v() {
        const e = h();
        e && e.classList.remove(w()), "classic" !== k() && (! function() {
            const e = document.querySelector(".cky-overlay");
            e && e.classList.add("cky-hide")
        }(), c._ckyGetFromStore("action") || _()), c._ckyGetFromStore("action") && j();
    ...
    function S() {
        const e = h();
        e && e.classList.add(w()), "classic" !== k() && (! function() {
            const e = document.querySelector(".cky-overlay");
            e && e.classList.remove("cky-hide")
        }(), b())
    }
    ...
    function C() {
        const e = h();
        e && e.classList.toggle(w()), "classic" !== k() && function() {
            const e = document.querySelector(".cky-overlay");
    ...
    function w() {
        return "classic" === k() ? "cky-consent-bar-expand" : "cky-modal-open"
    }
    ...
    function E(e = !1) {
        n._preferenceOriginTag = e, "classic" === k() ? C() : S()
    }

    function T(e, t) {
        const o = e.querySelector("[data-cky-tag=" + t + "]");
        if (!o) return !1;
        let c = o,
            r = "";
        o.getAttribute("src") || (c = o.querySelector("img")), r = c.src;
        const s = r.lastIndexOf("/"),
            i = r.substring(s + 1);
        c.src = n._assetsURL + i
    }
    ...
    function $({
        ...
        for (const {
            ...
            of e) t[n] && c._ckySetCookie(n, "", 0, o)
    }

    function A(e = !1) {
        for (const t of n._categories) {
            const o = c._ckyGetFromStore(t.slug);
            I(t, "yes" === o || !o && t.defaultConsent[p()] || t.isNecessary, t.isNecessary, n._shortCodes.find(e => "cky_category_toggle_label" === e.key).content.replace("[cky_preference_{{category_slug}}_title]", t.name), e), M(t)
        }
    }
        ...
        ["ckyCategoryDirect", "ckySwitch"].map(s => {
            ...
            u && (! function(e, t = {}, o = !1) {
                ...
                "detail-category-toggle" === e.parentElement.getAttribute("data-cky-tag") ? function(e, t) {
                    ...
                    t.isNecessary ? c && c.remove() : (r && r.remove(), ("classic" === k() && n._bannerConfig.config.categoryPreview.status || t.cookies && 0 === t.cookies.length) && c && c.remove())
    ...
    function N(e, t, o) {
        if (!e) return;
        const c = `cky_${t?"disable":"enable"}_category_label`,
            r = n._shortCodes.find(e => e.key === c);
        if (!r) return;
        const s = o.replace(/{{status}}/g, t ? "disable" : "enable").replace(`[${c}]`, r.content);
    ...
    function x(e = "custom") {
        return () => {
            ! function(e = "all") {
                const t = p();
                c._ckySetInStore("action", "yes"), "gdpr" === t ? c._ckySetInStore("consent", "reject" === e ? "no" : "yes") : c._ckySetInStore("consent", "yes");
                const o = {
                    accepted: [],
                    rejected: []
                };
                for (const r of n._categories) {
                    let n = "no";
                    n = "gdpr" === t ? r.isNecessary || "reject" !== e && ("custom" !== e || F(r.slug)) ? "yes" : "no" : r.defaultConsent.ccpa ? "yes" : "no", c._ckySetInStore("" + r.slug, n), "no" === n ? (o.rejected.push(r.slug), $(r)) : o.accepted.push(r.slug)
                }
                O()
            }(e), d(), v(),
                function() {
    ...
    }, "undefined" != typeof document && ("complete" !== document.readyState && "interactive" !== document.readyState ? document.addEventListener("DOMContentLoaded", g) : g());
    const q = document.createElement;

    function O() {
        if (1 === navigator.doNotTrack) return;
        const e = c._ckyGetFromStore("consent");
        ("gdpr" !== p() || e && "yes" === e) && (n._backupNodes = n._backupNodes.filter(({
            position: e,
            node: t,
            uniqueID: n
        }) => {
    ...
    const H = new MutationObserver((function(e) {
        for (const {
                addedNodes: t
            }
            of e)
            for (const e of t)
                if (e.src && e.nodeName && ["script", "iframe"].includes(e.nodeName.toLowerCase())) try {
                    const t = e.src.startsWith("//") ? `${window.location.protocol}${e.src}` : e.src,
                        ...
                        s = `${o}${r}`.replace(/^www./, "");
                    if (D(e, s), !P(s)) continue;
                    const i = c._ckyRandomString(8, !1);
                    "iframe" === e.nodeName.toLowerCase() ? R(e, i) : e.type = "javascript/blocked", e.remove(), n._backupNodes.push({
                        position: document.head.compareDocumentPosition(e) & Node.DOCUMENT_POSITION_CONTAINED_BY ? "head" : "body",
                        node: e.cloneNode(),
                        uniqueID: i
                    })
                } catch (e) {}
    ...
    function P(e) {
        const t = n._providersToBlock.find(t => new RegExp(t.re).test(e));
        return t && t.categories.some(e => B(e))
    }

    function G(e, t) {
        return e.hasAttribute("data-cookieyes") && B(e.getAttribute("data-cookieyes").replace("cookieyes-", "")) || P(t || e.src)
    }

    function F(e) {
        return ["ckySwitch", "ckyCategoryDirect"].some(t => {
